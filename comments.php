	...
	<?php if ( comments_open() ) : ?>
		...
		<?php else :  // 非必须登录 ?>
			<?php if(is_user_logged_in()) : $current_user = wp_get_current_user(); ?>
				...
				<textarea class="form-control" rows="3" id="comment" onkeydown="if(event.ctrlKey&&event.keyCode==13){document.getElementById('submit').click();return false};" placeholder="欢迎留言，广告绕道~~" tabindex="1" name="comment"></textarea>
			<?php else: ?>
				<!-- 非登录用户 -->
				<div class="comment-form-container" role="form">
					<h2 class="comment-reply-title" id="comment-reply-title">
